<?php
    ...
    HTML::macro('vtext', function($field, $value = null, $attrs = [] ) {
        ...
        $html = '';
        $html .= '<div class="form-group">';
        $html .= 	'<label for="'.$field.'" class="form-control-label">'. $label;
        if( isset($attrs['data-validation']) && $attrs['data-validation'] == 'required' ) $html .= '<strong style="color: red;"> * </strong>';
        $html .= '</label>';
        $html .= 		Form::text($field, $value, $attrs);
        if( isset($attrs['notes']) ) $html .= '<span class="form-text text-muted"><code>'. $attrs['notes'] .'</code></span>';
        $html .= '</div>';
        return $html;

    });

    HTML::macro('vdate', function($field, $value = null, $attrs = [] ) {

        if(! array_key_exists('label', $attrs) ) $attrs['label'] = ucfirst($field);
        if(! array_key_exists('placeholder', $attrs) ) $attrs['placeholder'] = 'Enter '. ucfirst($attrs['label']);
        if(! array_key_exists('id', $attrs) ) $attrs['id'] = $field;
        if(! array_key_exists('class', $attrs) ) $attrs['class'] = 'form-control datepicker';
    ...
    HTML::macro('vurl', function($field, $value = null, $attrs = [] ) {
        ...
        if( isset($attrs['data-validation']) && $attrs['data-validation'] == 'required' ) $html .= '<strong style="color: red;"> * </strong>';
        $html .= '</label>';
        $html .= 		Form::url($field, $value, $attrs);
        $html .= '</div>';
        return $html;
    ...
    HTML::macro('vemail', function($field, $value = null, $attrs = [] ) {
        ...
        $html .= '<div class="form-group">';
        $html .= 	'<label for="'.$field.'" class="form-control-label">'. $label;
        if( isset($attrs['data-validation']) && in_array($attrs['data-validation'], ['email', 'required']) ) $html .= '<strong style="color: red;"> * </strong>';
        $html .= '</label>';
        $html .= 		Form::email($field, $value, $attrs);
        $html .= '</div>';
        return $html;
    ...
    HTML::macro('vimage', function($field, $attrs = [] ) {
        ...
        $html .= 		Form::file($field, $attrs);
        $html .= '<label class="custom-file-label" for="'. $attrs['id'] .'">Choose file</label></div>';
        ...
        if( isset($attrs['value']) ) {
            ...
            $html .= '<a href="'.$value.'" class="light-image">';
            $html .= 	HTML::image( $value, $label, ['style' => 'width:50%; height%', 'class' => 'thumb-sm']);
            $html .= '</a>';
        } else {

            if( isset($attrs['type']) && $attrs['type'] == 'company' ) {
                $html .= 	HTML::image( 'uploads/company-default-logo.png', 'not-found', ['style' => 'width:50%; height%']);
            } else {
