...
namespace Zombata\Core\Repositories;
...
class Repository {
    ...
    public function getValueArray($attrs) {
        ...
        foreach( $attrs as $index => $value ) {
            if ($index == 'isAjax') continue;
            if($value != null || ($index == 'isAjax')) $arr[$index] = $value;
        }
        return $arr;
    }
    ...
    public function redirectBackWithSuccess( $msg, $routeName = null ) {
        ...
        if (filter_var($routeName, FILTER_VALIDATE_URL)) {
            return redirect()->to($routeName)->withSuccess($msg);
        }

        if( $routeName ) return redirect()->route($routeName)->withSuccess($msg);

        if( $val = request('save2') ) return redirect()->to($val)->withSuccess($msg);
